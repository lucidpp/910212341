...
let savedThemePref = 'grey';
try {
    // Respect stored preference if present, otherwise default to Grey
    savedThemePref = localStorage.getItem('chirpless_theme_pref') || 'grey';
} catch(e){ savedThemePref = 'grey'; }
...
document.getElementById('btn-play').onclick = () => {
    ...
    setThemeForAll('grey');
    try { const popup = playMenu.querySelector('.menu-popup'); if (popup) { popup.style.width = '100%'; popup.style.height = '100%'; popup.style.top = '0'; popup.style.left = '0'; popup.style.transform = 'none'; popup.style.maxWidth = 'none'; } } catch(e){}
    ...
    // Default open Games
    // Ensure Explore uses the grey theme and stretches smartly to the viewport
    setThemeForAll('grey');
    tabGames.click();
};
...
btnExit.onclick = () => {
    ...
    // Also prevent the next automatic pointer-lock / auto-rejoin click from immediately returning the user to the game
